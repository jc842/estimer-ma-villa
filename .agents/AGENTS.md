# Directives & Contraintes d'Environnement — estimer-ma-villa.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **estimer-ma-villa.com** (estimer-ma-villa) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `estimer-ma-villa.com` |
| **URL Cloudflare Pages** | `https://estimer-ma-villa.pages.dev` |
| **Dépôt GitHub** | `jc842/estimer-ma-villa` (branche principale : `main`) |
...
| **Matomo Site ID** | `106` |
| **Matomo MTM Container** | `container_mMunnISx.js` (Container ID : `mMunnISx`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `108` |
| **Zones Publicitaires** | Header (`654`), In-Content (`655`), Mobile Sticky (`656`), Large Rectangle (`653`), Half Page (`652`), Skyscraper (`657`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_mMunnISx.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 655).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 653).
